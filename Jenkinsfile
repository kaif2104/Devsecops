            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        if [ -d "./frontend/productfrontend/build" ]; then
                            scp -i ${SSH_KEY} -o StrictHostKeyChecking=no -r ./frontend/productfrontend/build/* ${WEB_SERVER_USER}@${WEB_SERVER_IP}:/var/www/frontend/
                            ssh -i ${SSH_KEY} -o StrictHostKeyChecking=no ${WEB_SERVER_USER}@${WEB_SERVER_IP} "sudo chown -R www-data:www-data /var/www/frontend && sudo chmod -R 755 /var/www/frontend"
                        fi
